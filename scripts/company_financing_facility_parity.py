#!/usr/bin/env python3
"""Compare exact reviewed financing-facility candidates with projections.

This validates facility identity, lifecycle balances, rate, interest evidence,
and the explicit absence of lender, cash, accounting, and close side effects.
"""
...
class FinancingParityError(RuntimeError):
    pass
...
def expected(receipt: dict[str, Any]) -> dict[tuple[str, str, str, str], list[dict[str, Any]]]:
    if receipt.get("cohort") != "financing-facility-v1":
        raise FinancingParityError("receipt is not a financing-facility cohort")
    result: dict[tuple[str, str, str, str], list[dict[str, Any]]] = defaultdict(list)
    for item in receipt.get("accepted_items", []):
        if not isinstance(item, dict) or item.get("target_type") != "financing_facility":
            raise FinancingParityError("financing receipt contains an invalid item")
        candidate = item.get("target_candidate")
        if not isinstance(candidate, dict):
            raise FinancingParityError("financing receipt candidate is not an object")
        key = (
    ...
    if not result:
        raise FinancingParityError("financing receipt contains no facilities")
    return result
...
def compare_rows(
    ...
    for aggregate_type, aggregate_id, payload in rows:
        ...
        if not isinstance(candidate, dict):
            raise FinancingParityError("financing projection candidate is not an object")
    ...
    return {
        "format": "moonproj.erp.financing-facility-parity.v1",
...
def sqlite_rows(database: Path) -> tuple[list[tuple[str, str, dict[str, Any]]], str]:
    ...
    for aggregate_type, aggregate_id, payload in rows:
        try:
            ...
            raise FinancingParityError("SQLite financing projection payload is invalid JSON") from error
        if not isinstance(value, dict):
            raise FinancingParityError("SQLite financing projection payload is not an object")
...
def postgres_rows(args: argparse.Namespace, receipt: dict[str, Any]) -> tuple[list[tuple[str, str, dict[str, Any]]], str]:
    ...
    try:
        ...
        raise FinancingParityError(str(error)) from error
    rows: list[tuple[str, str, dict[str, Any]]] = []
    for line in output.splitlines():
        ...
        fields = line.split("|")
        if len(fields) != 3:
            raise FinancingParityError("unexpected PostgreSQL financing parity output")
        try:
            payload = json.loads(bytes.fromhex(fields[2]).decode("utf-8"))
        except (ValueError, UnicodeDecodeError, json.JSONDecodeError) as error:
            raise FinancingParityError("PostgreSQL financing projection payload is invalid") from error
        if not isinstance(payload, dict):
            raise FinancingParityError("PostgreSQL financing projection payload is not an object")
...
def main() -> int:
    ...
    try:
        ...
        if args.backend == "sqlite":
            if args.database is None:
                raise FinancingParityError("SQLite backend requires --database")
    ...
    except (OSError, FinancingParityError, sqlite3.Error) as error:
        print(f"financing-facility parity failed: {error}", file=sys.stderr)
        return 1
