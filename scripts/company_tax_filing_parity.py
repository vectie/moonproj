#!/usr/bin/env python3
"""Compare exact reviewed tax-filing candidates with durable projections.

This gate checks identity and the complete tax/filing candidate, including
calculated amounts, rates, currency, filing state, and authority reference.
It supports the SQLite rehearsal and the PostgreSQL target adapter.
"""
...
class TaxParityError(RuntimeError):
    pass
...
def expected(receipt: dict[str, Any]) -> dict[tuple[str, str, str, str], list[dict[str, Any]]]:
    if receipt.get("cohort") != "tax-filing-v1":
        raise TaxParityError("receipt is not a tax-filing cohort")
    result: dict[tuple[str, str, str, str], list[dict[str, Any]]] = defaultdict(list)
    for item in receipt.get("accepted_items", []):
        if not isinstance(item, dict) or item.get("target_type") != "tax_filing":
            raise TaxParityError("tax receipt contains an invalid item")
        candidate = item.get("target_candidate")
        if not isinstance(candidate, dict):
            raise TaxParityError("tax receipt candidate is not an object")
        key = (
    ...
    if not result:
        raise TaxParityError("tax receipt contains no filings")
    return result
...
def compare_rows(
    ...
    for aggregate_type, aggregate_id, payload in rows:
        ...
        if not isinstance(candidate, dict):
            raise TaxParityError("tax projection candidate is not an object")
    ...
    return {
        "format": "moonproj.erp.tax-filing-parity.v1",
...
def sqlite_rows(database: Path) -> tuple[list[tuple[str, str, dict[str, Any]]], str]:
    ...
    for aggregate_type, aggregate_id, payload in rows:
        try:
            ...
            raise TaxParityError("SQLite tax projection payload is invalid JSON") from error
        if not isinstance(value, dict):
            raise TaxParityError("SQLite tax projection payload is not an object")
...
def postgres_rows(args: argparse.Namespace, receipt: dict[str, Any]) -> tuple[list[tuple[str, str, dict[str, Any]]], str]:
    ...
    try:
        ...
        raise TaxParityError(str(error)) from error
    rows: list[tuple[str, str, dict[str, Any]]] = []
    for line in output.splitlines():
        ...
        fields = line.split("|")
        if len(fields) != 3:
            raise TaxParityError("unexpected PostgreSQL tax parity output")
        try:
            payload = json.loads(bytes.fromhex(fields[2]).decode("utf-8"))
        except (ValueError, UnicodeDecodeError, json.JSONDecodeError) as error:
            raise TaxParityError("PostgreSQL tax projection payload is invalid") from error
        if not isinstance(payload, dict):
            raise TaxParityError("PostgreSQL tax projection payload is not an object")
...
def main() -> int:
    ...
    try:
        ...
        if args.backend == "sqlite":
            if args.database is None:
                raise TaxParityError("SQLite backend requires --database")
    ...
    except (OSError, TaxParityError, sqlite3.Error) as error:
        print(f"tax-filing parity failed: {error}", file=sys.stderr)
        return 1
