#!/usr/bin/env python3
"""Compare reviewed cash-plan and fund-dispatch candidates with projections."""
...
class TreasuryParityError(RuntimeError):
    pass
...
def expected(receipt: dict[str, Any]) -> dict[tuple[str, str, str, str], list[dict[str, Any]]]:
    if receipt.get("cohort") != "treasury-plan-dispatch-v1":
        raise TreasuryParityError("receipt is not a treasury plan/dispatch cohort")
    result: dict[tuple[str, str, str, str], list[dict[str, Any]]] = defaultdict(list)
    for item in receipt.get("accepted_items", []):
        if not isinstance(item, dict) or item.get("target_type") not in {"cash_plan", "fund_dispatch"}:
            raise TreasuryParityError("treasury receipt contains an invalid item")
        candidate = item.get("target_candidate")
        if not isinstance(candidate, dict):
            raise TreasuryParityError("treasury receipt candidate is not an object")
        key = (
    ...
    if not result:
        raise TreasuryParityError("treasury receipt contains no items")
    return result
...
def compare_rows(
    ...
    for aggregate_type, aggregate_id, payload in rows:
        ...
        if not isinstance(candidate, dict):
            raise TreasuryParityError("treasury projection candidate is not an object")
    ...
    return {
        "format": "moonproj.erp.treasury-plan-dispatch-parity.v1",
...
def sqlite_rows(database: Path) -> tuple[list[tuple[str, str, dict[str, Any]]], str]:
    ...
    for aggregate_type, aggregate_id, payload in rows:
        try:
            ...
            raise TreasuryParityError("SQLite treasury projection payload is invalid JSON") from error
        if not isinstance(value, dict):
            raise TreasuryParityError("SQLite treasury projection payload is not an object")
...
def postgres_rows(args: argparse.Namespace, receipt: dict[str, Any]) -> tuple[list[tuple[str, str, dict[str, Any]]], str]:
    ...
    try:
        ...
        raise TreasuryParityError(str(error)) from error
    rows: list[tuple[str, str, dict[str, Any]]] = []
    for line in output.splitlines():
        ...
        fields = line.split("|")
        if len(fields) != 3:
            raise TreasuryParityError("unexpected PostgreSQL treasury parity output")
        try:
            payload = json.loads(bytes.fromhex(fields[2]).decode("utf-8"))
        except (ValueError, UnicodeDecodeError, json.JSONDecodeError) as error:
            raise TreasuryParityError("PostgreSQL treasury projection payload is invalid") from error
        if not isinstance(payload, dict):
            raise TreasuryParityError("PostgreSQL treasury projection payload is not an object")
...
def main() -> int:
    ...
    try:
        ...
        if args.backend == "sqlite":
            if args.database is None:
                raise TreasuryParityError("SQLite backend requires --database")
    ...
    except (OSError, TreasuryParityError, sqlite3.Error) as error:
        print(f"treasury plan/dispatch parity failed: {error}", file=sys.stderr)
        return 1
