#!/usr/bin/env python3
"""Compare exact reviewed bank-statement candidates with durable projections.

The gate compares balances, currency, line count, line identities, amounts,
directions, and booked references. It intentionally does not claim cash or
...
def expected(receipt: dict[str, Any]) -> dict[tuple[str, str, str, str], list[dict[str, Any]]]:
    if receipt.get("cohort") != "bank-statement-v1":
        raise BankParityError("receipt is not a bank-statement cohort")
    result: dict[tuple[str, str, str, str], list[dict[str, Any]]] = defaultdict(list)
    for item in receipt.get("accepted_items", []):
        if not isinstance(item, dict) or item.get("target_type") != "bank_statement":
            raise BankParityError("bank receipt contains an invalid item")
        candidate = item.get("target_candidate")
        if not isinstance(candidate, dict):
            raise BankParityError("bank receipt candidate is not an object")
        key = (
    ...
    if not result:
        raise BankParityError("bank receipt contains no statements")
    return result
...
def compare_rows(
    ...
    for aggregate_type, aggregate_id, payload in rows:
        ...
        if not isinstance(candidate, dict):
            raise BankParityError("bank projection candidate is not an object")
    ...
    return {
        "format": "moonproj.erp.bank-statement-parity.v1",
...
def sqlite_rows(database: Path) -> tuple[list[tuple[str, str, dict[str, Any]]], str]:
    ...
    for aggregate_type, aggregate_id, payload in rows:
        try:
            ...
            raise BankParityError("SQLite bank projection payload is invalid JSON") from error
        if not isinstance(value, dict):
            raise BankParityError("SQLite bank projection payload is not an object")
...
def postgres_rows(args: argparse.Namespace, receipt: dict[str, Any]) -> tuple[list[tuple[str, str, dict[str, Any]]], str]:
    ...
    try:
        ...
        raise BankParityError(str(error)) from error
    rows: list[tuple[str, str, dict[str, Any]]] = []
    for line in output.splitlines():
        ...
        fields = line.split("|")
        if len(fields) != 3:
            raise BankParityError("unexpected PostgreSQL bank parity output")
        try:
            payload = json.loads(bytes.fromhex(fields[2]).decode("utf-8"))
        except (ValueError, UnicodeDecodeError, json.JSONDecodeError) as error:
            raise BankParityError("PostgreSQL bank projection payload is invalid") from error
        if not isinstance(payload, dict):
            raise BankParityError("PostgreSQL bank projection payload is not an object")
...
def main() -> int:
    ...
    try:
        ...
        if args.backend == "sqlite":
            if args.database is None:
                raise BankParityError("SQLite backend requires --database")
    ...
    except (OSError, BankParityError, sqlite3.Error) as error:
        print(f"bank-statement parity failed: {error}", file=sys.stderr)
        return 1
