#!/usr/bin/env python3
"""Compare reviewed invoice, receivable, and payable candidates with projections."""
...
class InvoiceParityError(RuntimeError):
    pass


TARGET_TYPES = {"invoice", "receivable", "payable"}
...
def expected(receipt: dict[str, Any]) -> dict[tuple[str, str, str, str], list[dict[str, Any]]]:
    if receipt.get("cohort") != "invoice-subledger-v1":
        raise InvoiceParityError("receipt is not an invoice-subledger cohort")
    ...
    for item in receipt.get("accepted_items", []):
        if not isinstance(item, dict) or item.get("target_type") not in TARGET_TYPES:
            raise InvoiceParityError("invoice receipt contains an invalid item")
        candidate = item.get("target_candidate")
        if not isinstance(candidate, dict):
            raise InvoiceParityError("invoice receipt candidate is not an object")
        key = (
    ...
    if not result:
        raise InvoiceParityError("invoice receipt contains no items")
    return result
...
def compare_rows(
    ...
    for aggregate_type, aggregate_id, payload in rows:
        ...
        if not isinstance(candidate, dict):
            raise InvoiceParityError("invoice projection candidate is not an object")
    ...
    return {
        "format": "moonproj.erp.invoice-subledger-parity.v1",
...
def sqlite_rows(database: Path) -> tuple[list[tuple[str, str, dict[str, Any]]], str]:
    ...
    for aggregate_type, aggregate_id, payload in rows:
        try:
            ...
            raise InvoiceParityError("SQLite invoice projection payload is invalid JSON") from error
        if not isinstance(value, dict):
            raise InvoiceParityError("SQLite invoice projection payload is not an object")
...
def postgres_rows(args: argparse.Namespace, receipt: dict[str, Any]) -> tuple[list[tuple[str, str, dict[str, Any]]], str]:
    ...
    try:
        ...
        raise InvoiceParityError(str(error)) from error
    rows: list[tuple[str, str, dict[str, Any]]] = []
    for line in output.splitlines():
        ...
        fields = line.split("|")
        if len(fields) != 3:
            raise InvoiceParityError("unexpected PostgreSQL invoice parity output")
        try:
            payload = json.loads(bytes.fromhex(fields[2]).decode("utf-8"))
        except (ValueError, UnicodeDecodeError, json.JSONDecodeError) as error:
            raise InvoiceParityError("PostgreSQL invoice projection payload is invalid") from error
        if not isinstance(payload, dict):
            raise InvoiceParityError("PostgreSQL invoice projection payload is not an object")
...
def main() -> int:
    ...
    try:
        ...
        if args.backend == "sqlite":
            if args.database is None:
                raise InvoiceParityError("SQLite backend requires --database")
    ...
    except (OSError, InvoiceParityError, sqlite3.Error) as error:
        print(f"invoice subledger parity failed: {error}", file=sys.stderr)
        return 1
