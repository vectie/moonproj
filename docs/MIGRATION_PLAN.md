...
scope coverage. Signed-user subscription create/update/delete aliases now
persist idempotent local `notification_subscription` projections with
source-shaped readback and tombstones, while imported
`sys_warning_subscription` rows remain read-only and delivery/provider/
accounting/cash/tax effects remain false. Message acknowledgement,
configuration writes, digest dispatch, provider calls, consent/retry policy,
