...
- Browser action delta: Rabbita `/loans/new` and command-owned `/loans/:guid` now mirror the source loan header fields, project/department context, dynamic draft/update/submit lifecycle, three-state balance display, and offset candidate. Loan detail now reads the source workflow instance by `processInstanceGuid` and renders the step/decision/assignee/comment trail, while imported loans remain read-only; multi-row offset history is source-backed, and attachment binaries plus cash/accounting/tax effects remain gated.
