...
- Browser action delta: Rabbita `/project/progress` now loads the project master list, scopes live ERP source progress/output rows plus overview reads to the selected project, mirrors progress/output/task-report tabs, provides source-shaped editable progress/report/output forms with project-scoped contract lookup for local command candidates, and exposes progress-row delete candidates only for command-owned rows. Imported rows remain read-only.
