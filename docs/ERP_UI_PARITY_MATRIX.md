...
- Browser action delta: Rabbita `/loans/new` and command-owned `/loans/:guid` now mirror the source loan header fields, project/department context, dynamic draft/update/submit lifecycle, three-state balance display, and offset candidate. The native PostgreSQL employee-advance boundary retains dynamic source fields and preserves imported loans as read-only; workflow timeline, multi-row offset history rendering, attachment binaries, and cash/accounting/tax effects remain gated.
...
| `/loans/:guid` | `../views/LoanDetail.vue` | `loan_editor_view` | `connected_loan_command_form` | `loan` | 7 / 10 | `connected_loan_command` | `accept_browser_loan_command_scenario_and_finance_owner` |
