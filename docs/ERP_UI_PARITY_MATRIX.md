...
- Browser action delta: the trusted native gateway now forwards project/contract import templates and batch commands; the authenticated route is covered for template download, dry-run validation, and committed project import. Contract import uses the same allow-listed boundary; browser CSV selection UI remains the next visual slice.
