...
- Browser action delta: Rabbita `/tender` now loads the project master list, scopes source tender reads and local tender creation to the selected project, and mirrors the source create-tender and award dialogs with editable fields, provider selection, and source-row selection. Per-row state controls and full award projection rendering remain open.
