...
| PostgreSQL target boundary | `POSTGRES_TARGET_SETUP.md`, `company_postgres_cohort_rehearsal.sh`, `company_postgres_target_apply.py`, `company_postgres_projection_apply.py`, `company_postgres_accounting_link_apply.py` | Complete for the available redacted and reviewed typed cohorts; a PostgreSQL 18 catalog-version-4 rehearsal owns 120 raw envelopes, 115 aggregate projections, 7 reviewed accounting-link records, and 24 cohort receipts before optional posting, with exact parity and idempotent replay. The optional reviewed accounting-posting cohort adds two immutable journal projections and a 21st parity report; the optional opening-control cohort adds five exact candidates and its own parity/replay report. Production provider ownership, opening-workbook acceptance, cash release, tax filing, and business ownership remain separate gates. |
