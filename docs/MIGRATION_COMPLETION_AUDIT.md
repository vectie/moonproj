...
| PostgreSQL target boundary | `POSTGRES_TARGET_SETUP.md`, `company_postgres_cohort_rehearsal.sh`, `company_postgres_target_apply.py`, `company_postgres_projection_apply.py`, `company_postgres_accounting_link_apply.py` | Complete for the available redacted and reviewed typed cohorts; a fresh PostgreSQL 18 catalog-version-4 rehearsal owns 120 raw envelopes, 115 aggregate projections, 7 reviewed accounting-link records, and 24 cohort receipts with exact parity and idempotent replay. The pre-existing target retains older append-only evidence under its prior mapping hashes; production posting, cash release, and business ownership remain separate gates. |
...
| Domain and finance controls | implementation packages and 245 tests | Complete for implemented slices; accounting links remain traceability, not posting. |
