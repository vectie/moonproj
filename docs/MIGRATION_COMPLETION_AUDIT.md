...
| Company browser surface | `frontend/main`, `frontend/public`, `company_postgres_read_model_server.py` | ERP-derived Rabbita shell/login/dashboard is complete and rendered in desktop and 390px mobile checks. Major ERP route families render source-shaped fixtures, representative detail/new forms are navigable, administrative routes retain distinct inbox, attachment, health, user, profile, notification, OCR, and webhook layouts, and the report center now opens a source-shaped read-only `/share/:token` cost-report preview. The dashboard successfully reads the configured PostgreSQL summary through the fixed development read-model API with an offline fixture fallback; command/mutation and production service boundaries remain open. |
