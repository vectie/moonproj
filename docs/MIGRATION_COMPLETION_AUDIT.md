...
| Domain and finance controls | implementation packages and 242 tests | Complete for implemented slices; accounting links remain traceability, not posting. |
...
| CBS budget control boundary | `finance/cbs`, `cmd/cbs_budget`, `ERP_CBS_BUDGET.md` | Complete as an opt-in reviewed boundary; a synthetic plan reserves and consumes a subject allocation, persists exact projection parity, and replays with zero inserts while accounting posting and cash release remain false. Real ERP budget allocations remain pending. |
...
| Warning evidence boundary | `intelligence/warning`, `cmd/warning`, `ERP_WARNING_BOUNDARY.md` | Complete locally as source-bound immutable warning projections; the checked-in plan proves native promotion, exact projection parity, and zero-insert replay while notification delivery, workflow mutation, and ticket persistence remain false/pending. |
