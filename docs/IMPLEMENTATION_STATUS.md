...
| ERP budget dictionary runtime | `scripts/company_postgres_service.py` + `scripts/company_postgres_service_smoke.py` + `ERP_BUDGET_RUNTIME_AUDIT.md` | Source-compatible cost-subject and proceedings reads preserve five enabled `my_biz_param_option` rows and three enabled `vys_proceeding` rows for the expense form. Expense list/detail, budget-check, user-scope, dictionary mutation, production identity, and finance-owner acceptance remain separate gates. |
| ERP investment runtime reads | `scripts/company_postgres_service.py` + `scripts/company_postgres_service_smoke.py` + `ERP_INVESTMENT_RUNTIME_AUDIT.md` | Source-compatible investment reads preserve one current feasibility version, 26 grouped indices across five dimensions, profit-summary values, and dimension metadata. Excel import, version/index mutation, valuation, cash, accounting, tax, browser identity, and investment-owner acceptance remain separate gates. |
| ERP admin governance runtime reads | `scripts/company_postgres_service.py` + `scripts/company_postgres_service_smoke.py` + `ERP_ADMIN_RUNTIME_AUDIT.md` | Source-compatible admin reads preserve one dictionary group with five options, two imported login audit rows, 29 health-table coverage rows, and an explicitly empty BPM pool. Super-user identity, full audit retention, dictionary writes, role administration, source completeness, and owner acceptance remain separate gates. |
