...
| Investment portfolio and valuation | `investment/portfolio` + `cmd/investment_benchmark` + `cmd/investment_performance` | Mandate-bound position book, exposure limits, explicit quote valuation, deterministic period-scoped per-position performance attribution and benchmark active-return comparison, source-snapshot/mapping/evidence-bound external benchmark reconciliation with tolerance and analytics-only projection markers, missing-evidence guards, and gain/loss mark-to-market source-to-journal adapters with explicit valuation authority; the reviewed performance cohort now persists portfolio, performance, and benchmark-reconciliation candidates with exact parity/replay, while cash and accounting-book posting remain separate. |
