...
| Employee advance/offset accounting-link cohort | `scripts/company_expense_advance_accounting_rehearsal.sh` + `scripts/fixtures/expense_advance_accounting_link_mapping.example.json` + `cmd/accounting_link` | Separate reviewed issuance and offset journal links pass native validation, exact SQLite/PostgreSQL identity parity, and zero-insert replay for two links. The expense claim remains outside this link slice to avoid duplicate recognition; book posting, cash, tax, period close, and owner acceptance remain separate. |
...
| Accounting/subledger reconciliation evidence | `scripts/company_sqlite_accounting_reconciliation.py` + `scripts/company_accounting_link_parity.py` + `scripts/erp_migration_rehearsal.sh` + `ERP_SETTLEMENT_ACCOUNTING.md` | Reconciles base, employee advance/offset, synthetic receivable-collection/payable-payment, invoice-opening, payable-opening, procurement-commitment, and valuation links back to promoted source candidates, principal, amount, currency, and durable event/source/journal rows; the dedicated cohorts add exact SQLite/PostgreSQL event identity parity. Reports remain traceability-only with `cash_released=false` and `period_posted=false`. |
