...
3. Complete remaining cross-domain persistence links: workflow assignment
   effective-dated delegation/SLA semantics and CBS configuration. Typed
   workflow-assignment attachment and non-authorizing projection evidence are
   implemented, as is a separately reviewed pending-posting delivery
   recognition projection (the available source row remains quarantined). The
   reviewed CBS cost-link cohort now covers the seven fixture
