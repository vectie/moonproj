...
| ERP project master runtime | `scripts/company_postgres_service.py` + `scripts/company_postgres_service_smoke.py` + `frontend/main` + `ERP_PROJECT_RUNTIME_AUDIT.md` | Source-preserving PostgreSQL reads now expose two projects, fourteen lifecycle instances, nine tasks, and task-report evidence through `/api/company/projects` and detail. Rabbita `/projects` and `/projects/:guid` show live rows with the designer layout retained as an explicit fallback; project mutations and plan mutations remain separate gates. |
