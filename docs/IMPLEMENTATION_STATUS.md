...
2. Connect one authenticated runtime vertical slice end to end—preferably
   ...
   visible UI state changes. The local expense command/audit boundary now
   passes its backend lifecycle smoke; same-origin session/token wiring and
   Rabbita state updates remain. Do not count fixture-backed buttons as
   complete behavior.
