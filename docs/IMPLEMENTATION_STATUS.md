...
| ERP admin governance runtime reads | `scripts/company_postgres_service.py` + `scripts/company_postgres_service_smoke.py` + `ERP_ADMIN_RUNTIME_AUDIT.md` | Source-compatible admin reads preserve one dictionary group with five options and two imported login audit rows through group/options and audit log/action endpoints. Super-user identity, full audit retention, dictionary writes, role administration, and owner acceptance remain separate gates. |
