...
| Native accounting-link receipt and durable apply | `scripts/erp_accounting_link_plan.py` + `cmd/accounting_link` + `scripts/company_sqlite_accounting_link_apply.py` | The allow-listed source-to-journal boundary now covers commitment, advance/offset, settlement, expense, delivery progress/recognition, receivable opening/collection, payable opening/payment, tax, financing, investment acquisition/valuation, asset, cash, and bank-statement source types. It still requires explicit candidate/mapping principal, amount, and currency equality; `investment_valuation` additionally requires exact native event, journal, account, scope, and event-type identity. The existing commitment/advance receipt persists three links transactionally, while synthetic settlement, receivable, delivery-recognition, tax, financing, and valuation smokes prove native receipt generation and missing-amount quarantine. It does not post cash or infer accounting policy. |
| PostgreSQL accounting-link cohort rehearsal | `scripts/company_postgres_accounting_cohort_rehearsal.sh` + `scripts/company_postgres_accounting_link_apply.py` + `scripts/company_investment_valuation_accounting_rehearsal.sh` | Runs any reviewed domain receipt through the allow-listed planner, native validator, PostgreSQL traceability apply, exact identity parity, and idempotent replay. The dedicated valuation rehearsal appends one link to the existing target (8 links total in that local run) and replays zero; the main typed rehearsal remains at 7 links unless this separate accounting cohort is supplied. |
...
| Accounting/subledger reconciliation evidence | `scripts/company_sqlite_accounting_reconciliation.py` + `scripts/company_accounting_link_parity.py` + `scripts/erp_migration_rehearsal.sh` + `ERP_SETTLEMENT_ACCOUNTING.md` | Reconciles base, advance/offset, and synthetic receivable-collection/payable-payment links back to promoted source candidates, principal, amount, currency, and durable event/source/journal rows; the valuation cohort adds exact SQLite/PostgreSQL event identity parity. Reports remain traceability-only with `cash_released=false` and `period_posted=false`. |
...
4. Extend the reviewed accounting-link/subledger reconciliation gate from the
   ...
   source-to-journal links for those source cohorts remain to be reconciled;
   the reviewed investment valuation event/link now has exact SQLite/PostgreSQL
   parity and replay evidence. Opening receivable/payable recognition links are
   covered, while production source acceptance remains open.
