...
2. Extend the connected runtime vertical slice end to end—preferably
   ...
   expense create/submit path now proves same-origin gateway forwarding and
   visible state updates on top of the backend's idempotent command/audit
   lifecycle. Finish the local reject/resubmit/approve controls, replace the
   fixed development identity with a reviewed session/token boundary, and
   attach browser evidence before calling the slice accepted. Do not count
   fixture-backed buttons as complete behavior.
