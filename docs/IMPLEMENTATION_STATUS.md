...
| PostgreSQL typed-cohort rehearsal | `scripts/company_postgres_cohort_rehearsal.sh` + `scripts/company_postgres_projection_parity.py` | Re-runs raw staging, ten core typed cohorts, and optional CBS, workflow-assignment, delivery-progress, advance-offset, and payment-accounting cohorts against PostgreSQL, compares every receipt by target/source identity, and replays each cohort. The configured local target reports 120 raw records, 109 aggregate projections, 7 accounting links, and 19 receipts with zero replay inserts. |
| Rabbita ERP UI clone | `frontend/main` + `frontend/public` + `frontend/README.md` | Designer-facing ERP shell is ported with Rabbita 0.12.4 and Warren-compatible JS build output: source login gradient/copy, 220/64px dark navigation, nested ERP menu hierarchy, header actions, dashboard KPI/funnel/risk layout, and mobile drawer behavior. Major ERP route families render source-shaped fixtures for projects/plans/workflow, AI, sales, cost/procurement, finance, analysis, and system administration; representative project, contract, expense, loan, and supplier detail/new flows now open as responsive forms. The dashboard calls the fixed PostgreSQL read-model summary endpoint with an offline snapshot fallback; command/mutation API wiring and final page-by-page parity remain pending. |
...
| Native accounting-link receipt and durable apply | `scripts/erp_accounting_link_plan.py` + `cmd/accounting_link` + `scripts/company_sqlite_accounting_link_apply.py` | An explicit source-to-journal map validates two commitment journals plus one employee-advance opening journal under append authority, emits a native receipt, persists three event/source/journal links transactionally as `AccountingLinked`, reopens with integrity `ok`, and replays with zero inserts. It does not post cash or infer accounting policy. |
...
| Accounting/subledger reconciliation evidence | `scripts/company_sqlite_accounting_reconciliation.py` + `scripts/erp_migration_rehearsal.sh` | Reconciles 3 base links and 2 advance/offset links back to promoted source candidates, principal, amount, currency, and durable event/source/journal rows; reports `reconciled` while explicitly keeping `cash_released=false` and `period_posted=false`. |
...
| Receivable | `finance/receivable` | Open, partial, complete, and over-collection-controlled customer balances, with an explicit opening receivable-to-revenue accounting-event adapter. |
...
| Payable | `finance/payable` | Supplier obligation, partial/full payment, overpayment, and void controls, with an explicit opening expense-to-payable accounting-event adapter. |
...
| Investment portfolio and valuation | `investment/portfolio` | Mandate-bound position book, exposure limits, explicit quote valuation, and missing-evidence guards. |
...
| Tax obligation | `finance/tax` | Rate-based calculation, review, filing, payment, voiding, and authority-scoped events. |
...
6. Add investment formula semantics, performance attribution, and accounting
   events around the persistent model, mandate, proposal, position, and risk
   projections.
