...
`authorizing=false`. It does not create `sys_warning` rows, resolve warnings,
send notifications, or create/modify tickets.
...
The designer warning table remains only a transport-failure fallback. Rule
configuration, scans, tickets, notification delivery, production identity, and
