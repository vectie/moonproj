...
| ERP webhook configuration runtime routes | Three-platform Feishu/DingTalk/WeCom webhook configuration metadata and candidate update | `cmd/postgres_company_service/webhook_commands.mbt` + `scripts/company_postgres_webhook_smoke.sh` + `frontend/main` | `/api/company/webhook/config` and `/source` preserve the source `sys_param` shape with redacted URL/secret status and explicit coverage; signed super-user `PUT /config/:platform` candidates preserve enabled/optional URL/secret/`__keep__` semantics with idempotent redacted projections and audit evidence, but do not bind credentials or deliver. The current export has no `sys_param` rows, so Rabbita `/webhook-config` renders empty-source state after a successful read. Managed credential storage, provider delivery, overdue scans, production identity, and owner acceptance remain gates. See `docs/ERP_WEBHOOK_RUNTIME_AUDIT.md`. |
