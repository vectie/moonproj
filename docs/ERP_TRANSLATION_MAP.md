...
| ERP AI Hub runtime routes | AI workbench usage, draft history, query history, correction logs, and correction-rate observation | `intelligence/ai` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `frontend/main` | Source-compatible `/api/company/ai-hub/{usage-stats,drafts,drafts/:draftId,query-log,corrections,correction-stats}` preserve the source response shapes with explicit coverage and redacted draft/query payloads. The current export has no AI Hub draft/query/correction/session rows, so Rabbita `/ai-hub` loads successful empty-source metrics/history while retaining the designer workbench. Intake, confirmation, discard, natural-language query, explanation, rule, approval-draft, global-ask, session, command, provider execution, prompt retention, and draft promotion remain gated. See `docs/ERP_AI_RUNTIME_AUDIT.md`. |
