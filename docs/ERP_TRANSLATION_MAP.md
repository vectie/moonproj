...
| ERP attachment runtime routes | Attachment metadata, AI extraction status, and evidence linkage | `foundation/evidence` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `frontend/main` | `/api/company/attachments/{list,all,stats}` preserve imported metadata and source coverage; the current export has no `attachment` rows, so Rabbita `/attachments` renders an explicit empty source state. Binary storage/download, upload, delete, OCR re-extraction, retention, and authorization remain separate gates. See `docs/ERP_ATTACHMENT_RUNTIME_AUDIT.md`. |
