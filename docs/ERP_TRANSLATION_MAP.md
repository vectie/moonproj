...
| ERP admin governance runtime routes | Dictionary groups/options and audit log/action reads | `foundation/access` + `intelligence/notification` + `scripts/company_postgres_service.py` | Source-compatible admin reads preserve one dictionary group, five options, two audit rows, and action counts. Super-user scope, dictionary/audit writes, role changes, and retention remain separate. See `docs/ERP_ADMIN_RUNTIME_AUDIT.md`. |
