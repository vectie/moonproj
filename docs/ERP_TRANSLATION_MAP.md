...
| `cb_contract` | Commercial commitment | `operations/commitment` + `migration/erp` + `scripts/company_postgres_service.py` | Commitment state machine implemented; promotion requires explicit principal, project-scope, counterparty, and amount-bounded authority mappings. Source contract update/void aliases now reuse a command-owned PostgreSQL contract projection, preserve source-shaped fields and tombstone readback, and keep imported contracts read-only; command-owned milestone create/update/trigger/void aliases now extend the same source-shaped contract boundary with immutable revisions and no external effects. Source contract creation, CBS/budget validation, payment execution, and accounting/cash/tax effects remain separate gates. A separate reviewed accounting-link plan can persist source-to-journal identity without posting cash. |
...
| ERP MDM organization runtime routes | Business-unit hierarchy and legal-name evidence | `operations/mdm` + `scripts/company_postgres_service.py` | `/api/company/business-units/tree` preserves one group, two companies, four departments, and seven source rows with parent/child structure; organization mutations and ownership mapping remain separate. See `docs/ERP_MDM_RUNTIME_AUDIT.md`. |
