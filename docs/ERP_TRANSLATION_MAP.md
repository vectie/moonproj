...
| `cb_htfkplan`, `cb_htfk_apply`, `vcb_loan_simple`, `vcb_expense`, `sale_revenue` | Cashflow forecast and liquidity evidence | `finance/treasury` + `scripts/company_postgres_service.py` | Source-compatible `/api/company/cashflow/forecast`, `/forecast-v3`, `/forecast/detail`, `/inflow`, `/net`, and `/gap-alert` reads plus Rabbita `/cashflow` aggregate source planned, pending, approved, expense, loan, and revenue evidence. Every response carries source coverage and `authorizing=false`; missing sales/milestone/CBS rows remain explicit empty-source state. Cash release, bank settlement, accounting, tax, and AI explanation remain separate. |
...
| ERP investment runtime routes | Feasibility versions, plan indices, profit summary, and dimensions | `investment/portfolio` + `cmd/postgres_company_service/investment_commands.mbt` + `cmd/postgres_company_service/investment_excel.mbt` | Source-compatible investment reads preserve one current version, 26 indices in five dimensions, source profit values, and dimension metadata. Signed local version/index lifecycle commands now persist idempotent candidate projections and audit evidence while protecting imported rows; Excel upload/import, valuation, cash, accounting, and tax remain separately authorized. See `docs/ERP_INVESTMENT_RUNTIME_AUDIT.md`. |
