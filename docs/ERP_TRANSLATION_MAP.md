...
| ERP admin governance runtime routes | Dictionary groups/options, audit log/actions, and health coverage reads | `foundation/access` + `intelligence/notification` + `scripts/company_postgres_service.py` + `scripts/company_postgres_dev_gateway.py` | Source-compatible admin reads preserve one dictionary group, five options, two audit rows, action counts, 29 health-table rows, and an explicitly non-authorizing empty BPM pool. Dictionary POST/PATCH now use an authenticated, idempotent command-owned overlay that preserves imported rows and writes only projection/receipt/audit evidence; browser action wiring, super-user scope, audit writes/retention, role changes, and source completeness remain separate. See `docs/ERP_ADMIN_RUNTIME_AUDIT.md`. |
