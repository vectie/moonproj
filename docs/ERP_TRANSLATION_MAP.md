...
| ERP delivery/progress runtime routes | Project progress, output value, and task-report runtime | `operations/delivery` + `operations/project` + `scripts/company_postgres_service.py` + `frontend/main` | Native progress/deliverable state machines and draft/recognition cohorts are tested, but source `/progress` (7 handlers) and `/plan` (9 handlers) are not connected to the PostgreSQL service or Rabbita. `/project/progress` and `/project-plan` remain fixture-backed/read-only. Add source-preserving reads before authority-checked commands; keep acceptance, output confirmation, cost, recognition, cash, tax, and close separate. See `docs/ERP_DELIVERY_RUNTIME_AUDIT.md`. |
