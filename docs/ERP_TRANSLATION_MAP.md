...
| `cb_htfkplan`, `cb_htfk_apply`, `vcb_loan_simple`, `vcb_expense`, `sale_revenue` | Cashflow forecast and liquidity evidence | `finance/treasury` + `scripts/company_postgres_service.py` | Source-compatible `/api/company/cashflow/forecast` and Rabbita `/cashflow` aggregate monthly planned, pending, approved, expense, and loan outflow from imported envelopes, preserving explicit missing sales/expense/CBS coverage. Cash release, bank settlement, accounting, tax, and forecast-v3 semantics remain separate. |
