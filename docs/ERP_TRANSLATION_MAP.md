...
| `sys_warning`, warning rules, scans, tickets | Quality warning, exception observation, and follow-up | `governance/quality` + `scripts/company_postgres_service.py` | Source-compatible observed `/api/company/warning/*` reads evaluate twelve ERP rule definitions over imported envelopes, preserve one W005 source observation in the current export, and mark results `persisted=false`/`authorizing=false`. Signed warning-to-ticket now persists a command-owned `warning_ticket` candidate and `/tickets/mine` merges imported/local rows with assignee filtering; notification delivery, provider execution, and production ownership remain gated. |
| ERP warning-rule governance routes | Built-in/custom warning configuration, scan preview, and ticket candidates | `governance/quality` + `cmd/postgres_company_service/warning.mbt` + `scripts/company_postgres_warning_smoke.sh` | Signed super-user `PATCH /api/company/warning/rules/:code`, `POST/DELETE /custom-rules[/:code]`, dry-run `POST /scan`, and signed `POST /:guid/to-ticket` (plus `/source` aliases) persist idempotent redacted rule/ticket projections and audit receipts or return deterministic source coverage. Custom SQL is never executed, scan preview never persists findings, and ticket creation never sends notifications/webhooks or invokes an external ticket provider; production scans, ticket transitions, and warning ownership remain gates. |
