...
| Employee advance/offset accounting-link cohort | Advance issuance and expense-offset journal traceability | `scripts/company_expense_advance_accounting_rehearsal.sh` + `scripts/fixtures/expense_advance_accounting_link_mapping.example.json` + `cmd/accounting_link` | A separate reviewed map binds only the advance issuance and offset identities to two explicit journals with exact SQLite/PostgreSQL identity parity and zero-insert replay. The expense recognition event is intentionally not linked here, avoiding duplicate expense recognition; book posting, cash, tax, and period close remain separate. |
