...
master list through `/api/company/srm/providers`, preserving source coverage
and an explicit empty-source state without merging it into local command
projections. Local supplier create/update/review/blacklist/void commands,
...
| `srm_provider`, `srm_provider_bu` | Supplier master, qualification, blacklist | `operations/procurement` + `scripts/company_postgres_service.py` | Supplier identity, review, qualification, suspension, and blacklist controls implemented; the reviewed procurement cohort now persists qualified supplier projections with exact parity/replay, and `/srm/providers` reads those candidates through PostgreSQL. `/api/company/srm/risk-board` separately reproduces the ERP risk formula from imported supplier/contract/milestone envelopes and reports explicit source coverage without authorizing changes. Supplier creation/review/blacklist commands and real supplier rows remain pending. |
