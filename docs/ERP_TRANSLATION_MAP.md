...
| `tzsy_*` investment model | Investment assumptions and scenarios | `investment/model` + `investment/domain` + `investment/analytics` + `investment/portfolio` + `cmd/investment_benchmark` + `cmd/investment_model_eval` + `migration/erp` + `cmd/promote` | The real fixture promotes 1 version/26 indexes under explicit authority, preserves source value representations, and now evaluates numeric/date values, three parent totals, and four known ratio derivations into a source-bound analytics-only projection; deterministic period performance attribution and reviewed external benchmark comparison are local, while source-feed acceptance, richer formula vocabulary, and full accounting remain pending; acquisition and mark-to-market valuation links are explicit local events. |
