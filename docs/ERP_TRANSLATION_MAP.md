...
| ERP warning-rule governance routes | Built-in/custom warning configuration, scan preview, and ticket candidates | `governance/quality` + `cmd/postgres_company_service/warning.mbt` + `cmd/postgres_company_service/warning_ticket_commands.mbt` + `scripts/company_postgres_warning_smoke.sh` | Signed super-user `PATCH /api/company/warning/rules/:code`, `POST/DELETE /custom-rules[/:code]`, dry-run `POST /scan`, signed `POST /:guid/to-ticket`, and signed `PATCH /tickets/:id/{status,reassign,extend}` (plus `/source` aliases) persist idempotent redacted rule/ticket projections and audit receipts or return deterministic source coverage. Custom SQL is never executed, scan preview never persists findings, and ticket commands never send notifications/webhooks or invoke an external ticket provider; completing a local ticket also writes a warning-state resolution overlay. Production scans, external delivery, and warning-owner acceptance remain gates. |
