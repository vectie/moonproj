...
| ERP webhook configuration runtime routes | Three-platform Feishu/DingTalk/WeCom webhook configuration metadata | `intelligence/notification` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `frontend/main` | `/api/company/webhook/config` preserves the source `sys_param` shape with redacted URL/secret status and explicit coverage; the current export has no `sys_param` rows, so Rabbita `/webhook-config` renders empty-source state after a successful read. Configuration writes, provider delivery, overdue scans, production identity, and owner acceptance remain gates. See `docs/ERP_WEBHOOK_RUNTIME_AUDIT.md`. |
