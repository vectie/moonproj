...
| `srm_provider`, `srm_provider_bu` | Supplier master, qualification, blacklist, and risk-rescore command | `operations/procurement` + `cmd/postgres_company_service/supplier_rescore_commands.mbt` + `scripts/company_postgres_supplier_smoke.sh` | Supplier identity, review, qualification, suspension, and blacklist controls implemented; the reviewed procurement cohort persists qualified supplier projections with exact parity/replay, and `/srm/providers` reads those candidates through PostgreSQL. Signed `POST /api/company/srm/providers/rescore-all` (and `/source` alias) now computes the ERP risk formula, persists score/rating/tags only on command-owned supplier projections, protects imported providers, and records replayable command/audit receipts. `/api/company/srm/providers/:guid/risk` and `/api/company/srm/risk-board` read those persisted command risk values with explicit source coverage and no provider/cash/accounting/tax effects. Source provider CRUD remains command-owned and imported providers remain read-only; populated-source qualification, signature, external execution, and real supplier-row promotion remain owner/source-evidence gates. |
