...
| ERP investment runtime routes | Feasibility versions, plan indices, profit summary, and dimensions | `investment/portfolio` + `scripts/company_postgres_service.py` | Source-compatible investment reads preserve one current version, 26 indices in five dimensions, source profit values, and dimension metadata. Excel import, activation, index edits, valuation, cash, accounting, and tax remain separate. See `docs/ERP_INVESTMENT_RUNTIME_AUDIT.md`. |
