...
| `sys_warning`, warning rules, scans, tickets | Quality warning, exception observation, and follow-up | `governance/quality` + `scripts/company_postgres_service.py` | Source-compatible observed `/api/company/warning/*` reads evaluate twelve ERP rule definitions over imported envelopes, preserve one W005 source observation in the current export, and mark results `persisted=false`/`authorizing=false`. Warning/ticket writes, notification delivery, and production ownership remain gated. |
| ERP warning-rule governance routes | Built-in/custom warning configuration and scan preview candidates | `governance/quality` + `cmd/postgres_company_service/warning.mbt` + `scripts/company_postgres_warning_smoke.sh` | Signed super-user `PATCH /api/company/warning/rules/:code`, `POST/DELETE /custom-rules[/:code]`, and dry-run `POST /scan` (plus `/source` aliases) persist idempotent rule projections/audits or return deterministic source coverage. Custom SQL is never executed and scan preview never persists findings; production scans, tickets, notification delivery, and warning ownership remain gates. |
