...
| ERP AI Hub runtime routes | AI workbench usage, draft history, query history, correction logs, correction-rate observation, and explanation candidate | `intelligence/ai` + `cmd/postgres_company_service/ai_hub_commands.mbt` + `scripts/company_postgres_ai_hub_smoke.sh` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `frontend/main` | Source-compatible `/api/company/ai-hub/{usage-stats,drafts,drafts/:draftId,query-log,corrections,correction-stats}` preserve the source response shapes with explicit coverage and redacted draft/query payloads. Signed `POST /api/company/ai-hub/explain` (and `/source`) now returns a deterministic table summary with `provider_execution=false`, `persisted=false`, and `authorizing=false`; no LLM or prompt persistence occurs. The current export has no AI Hub draft/query/correction/session rows, so Rabbita `/ai-hub` loads successful empty-source metrics/history while retaining the designer workbench. Intake, confirmation, discard, natural-language query, rule, approval-draft, global-ask, session, command, provider-backed explanation, prompt retention, and draft promotion remain gated. See `docs/ERP_AI_RUNTIME_AUDIT.md`. |
