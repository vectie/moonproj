...
| ERP attachment runtime routes | Attachment metadata, AI extraction status, evidence linkage, and OCR re-extraction candidate | `foundation/evidence` + `cmd/postgres_company_service/attachment_commands.mbt` + `scripts/company_postgres_attachment_smoke.sh` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `frontend/main` | `/api/company/attachments/{list,all,stats}` preserve imported metadata and source coverage; the current export has no `attachment` rows, so Rabbita `/attachments` renders an explicit empty source state. Signed `POST /api/company/attachments/re-extract/:guid` (and `/source`) preserves the source 404 boundary and, for an imported row, returns a dry-run candidate without reading binaries, invoking OCR, or persisting metadata. Binary storage/download, upload, delete, provider-backed OCR, retention, and authorization remain separate gates. See `docs/ERP_ATTACHMENT_RUNTIME_AUDIT.md`. |
