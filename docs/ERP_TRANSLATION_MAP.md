...
| ERP AI analytics runtime routes | Confirmed AI drafts, query activity, corrections, workflow auto-skips, and AI-origin badge evidence | `intelligence/ai` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `frontend/main` | `/api/company/ai-stats/{overview,activity,badge}` preserve source analytics and optional user labels with explicit coverage, `authorizing=false`, `persisted=false`, and `provider_execution=false`. The current export has no AI analytics rows, so Rabbita `/ai-stats` renders empty-source state after successful reads. LLM/OCR execution, draft promotion, workflow authority, prompt retention, and owner acceptance remain gates. See `docs/ERP_AI_RUNTIME_AUDIT.md`. |
