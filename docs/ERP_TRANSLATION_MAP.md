...
| ERP project master/plan runtime routes | Project master, lifecycle, task, report, key-node summary, and delay-impact evidence | `operations/project` + `operations/delivery` + `cmd/postgres_company_service` + `scripts/company_postgres_project_plan_smoke.sh` + `frontend/main` | `/api/company/projects` and detail preserve two source projects, fourteen lifecycle instances, nine tasks, and one task report; source-compatible lifecycle/plan reads preserve seven ordered stages, seven tasks, one report, five key nodes, and downstream delay impact. Local project-plan task create/update/delete and evidence-gated report commands persist separate PostgreSQL projections with source/command provenance; imported task state is not mutated. Rabbita `/projects` and `/projects/:guid` show the live rows while retaining the designer fallback. AI scheduling, browser identity, and operations-owner acceptance remain separate gates. See `docs/ERP_PROJECT_RUNTIME_AUDIT.md`. |
