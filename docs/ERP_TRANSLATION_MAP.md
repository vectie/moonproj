...
| ERP warning-rule governance routes | Built-in warning rule enable/disable candidates | `governance/quality` + `cmd/postgres_company_service/warning.mbt` + `scripts/company_postgres_warning_smoke.sh` | Signed super-user `PATCH /api/company/warning/rules/:code` (and `/source` alias) persists idempotent `warning_rule_config` projections and audits, overlays the source-shaped rule read, and never runs scans or providers. Custom SQL rules, scan execution, tickets, and production warning ownership remain gates. |
