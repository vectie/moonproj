...
| ERP notification runtime routes | Company-owned messages, subscriptions, redacted notification config, email metadata, digest evidence, and provider discovery | `intelligence/notification` + `cmd/postgres_company_service/notification_commands.mbt` + `cmd/postgres_company_service/notification_dispatch_commands.mbt` + `scripts/company_postgres_notification_smoke.sh` + `frontend/main` | `/api/company/notify/{messages,messages/unread-count,subscriptions,config,email-outbox,digest/preview,digest/log,llm-providers}` preserve user-scoped message/unread reads, subscription rows, redacted config status, email metadata, digest evidence, and provider discovery with explicit source coverage. Signed-user source subscription create/update/delete aliases and message read/read-all aliases now persist idempotent local projections with source-shaped readback, replay/audit evidence, tombstones, and imported-message read overlays; signed super-user `POST /digest/dispatch` candidates persist bounded warning/recipient receipts, and `POST /config/test-webhook` candidates persist effective URL/kind state with `wouldSend`/skip reasons; both remain `dryRun=true`, `sent=false`/no delivery and no provider call. Imported `sys_warning_subscription` and `sys_message` rows remain read-only and delivery/provider/accounting/cash/tax effects remain false. The current export has no notification rows, so Rabbita `/inbox` and `/notify-config` render empty source states after successful reads. Configuration writes, delivery, provider execution, consent, and owner acceptance remain gates. See `docs/ERP_NOTIFICATION_RUNTIME_AUDIT.md`. |
