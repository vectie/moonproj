...
| ERP warning-rule governance routes | Built-in and custom warning rule configuration candidates | `governance/quality` + `cmd/postgres_company_service/warning.mbt` + `scripts/company_postgres_warning_smoke.sh` | Signed super-user `PATCH /api/company/warning/rules/:code` plus `POST/DELETE /custom-rules[/:code]` (and `/source` aliases) persist idempotent `warning_rule_config`/`warning_custom_rule` projections and audits, overlay source-shaped reads, and retain only SQL digests. Custom SQL is never executed; scans, tickets, notification delivery, and production warning ownership remain gates. |
