...
| ERP webhook configuration, test candidate, and overdue-preview runtime routes | Three-platform Feishu/DingTalk/WeCom webhook configuration metadata, candidate update/test, and dry-run overdue preview | `cmd/postgres_company_service/webhook_commands.mbt` + `cmd/postgres_company_service/webhook_delivery_commands.mbt` + `scripts/company_postgres_webhook_smoke.sh` + `frontend/main` | `/api/company/webhook/config` and `/source` preserve the source `sys_param` shape with redacted URL/secret status and explicit coverage; signed super-user `PUT /config/:platform` candidates preserve enabled/optional URL/secret/`__keep__` semantics with idempotent redacted projections and audit evidence, but do not bind credentials or deliver. Signed `POST /test/:platform` candidates validate effective platform state and persist redacted dry-run receipts with `wouldSend`/skip reasons while provider execution remains false. `/scan-overdue/preview` reads imported warning-ticket evidence and preserves no-overdue/no-platform/bounded-preview states without mutation or provider calls. The current export has no `sys_param` or warning-ticket rows, so Rabbita `/webhook-config` renders empty-source state after a successful read. Managed credential storage, actual provider delivery, real overdue scans, production identity, and owner acceptance remain gates. See `docs/ERP_WEBHOOK_RUNTIME_AUDIT.md`. |
