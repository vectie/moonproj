...
merging imported rows into local command state. Source CRUD aliases now merge
command-owned provider readback with explicit provenance. Local supplier
...
| `cb_contract` | Commercial commitment | `operations/commitment` + `migration/erp` + `scripts/company_postgres_service.py` | Commitment state machine implemented; promotion requires explicit principal, project-scope, counterparty, and amount-bounded authority mappings. Source contract `POST` create plus update/void aliases now reuse a command-owned PostgreSQL contract projection, preserve BU/project/provider/amount/CBS fields in source-shaped readback, and keep imported contracts read-only; command-owned milestone create/update/trigger/void aliases extend the same boundary with immutable revisions and no external effects. CBS/budget validation, payment execution, and accounting/cash/tax effects remain separate gates. A separate reviewed accounting-link plan can persist source-to-journal identity without posting cash. |
...
| `srm_provider`, `srm_provider_bu` | Supplier master, qualification, blacklist, and risk-rescore candidate | `operations/procurement` + `cmd/postgres_company_service/supplier_rescore_commands.mbt` + `scripts/company_postgres_supplier_smoke.sh` | Supplier identity, review, qualification, suspension, and blacklist controls implemented; the reviewed procurement cohort persists qualified supplier projections with exact parity/replay, and `/srm/providers` reads those candidates through PostgreSQL. `/api/company/srm/providers/:guid/risk` and `/api/company/srm/risk-board` reproduce the ERP risk formula from imported supplier/contract/milestone envelopes and report explicit source coverage without authorizing changes. Signed `POST /api/company/srm/providers/rescore-all` (and `/source` alias) now reports total/would-update counts as a dry-run candidate without rating writes, provider execution, or persistence. Source `POST/PATCH/PUT/DELETE /srm/providers` translate provider fields into idempotent command-owned supplier projections and source-shaped list/detail readback; imported providers remain read-only, and populated-source qualification, signature, rescore, and real supplier-row promotion remain owner/source-evidence gates. |
| `tender_plan`, `tender_award` | Tender planning, bidding, and award | `operations/procurement` + `cmd/postgres_company_service/main.mbt` | Planning → publishing → bidding → award → completion flow with qualified-supplier and duplicate-bid guards; the reviewed procurement cohort persists bid/award evidence and a separate performed commitment without settlement. The Rabbita `/tender` route reads PostgreSQL tender projections and drives local lifecycle commands; source-compatible create, forward-only state, qualification-gated award, split, and command-owned delete aliases translate ERP field names into local command projections and merge them back with provenance. Arbitrary state overwrite, hard-delete, award-to-commitment, and imported tender deletion remain separate gates. |
...
| `cb_contract_milestone` | Time/progress/event obligation | `operations/contract` + `scripts/company_postgres_service.py` | Milestone trigger, eligibility, achievement, payment, overdue, and cancellation states implemented. The PostgreSQL service now translates source single/batch create, update, delete/tombstone, event-trigger, and early-payment-check semantics into a command-owned projection; imported milestone rows remain read-only and command effects are explicitly cash/accounting/tax-neutral. Workflow-driven progress, payment execution, and owner acceptance remain separate gates. |
| milestone snapshots and payment events | Contract milestone persistence | `operations/contract` + `persistence/store` + `scripts/company_postgres_service.py` | Plan/actual amounts, triggers, and reached/paid state serialize as revisioned projections. Source-shaped contract detail merges command-owned milestones with provenance, while tombstones prevent resurfacing deleted revisions and immutable audit receipts retain actor/idempotency evidence. |
| `cb_htfkplan`, `cb_htfk_apply` | Payment plan and application | `operations/contract` + `operations/settlement` + `migration/erp` + `cmd/promote` + `scripts/company_postgres_service.py` | The real fixture promotes 4 planned milestones and 3 requested settlements after an explicit two-contract performed-state replay; reached milestones retain their ID on requested settlements and on separate immutable projections, approval/payment flags remain evidence, and release plus accounting-event links require separate target authority. The source payment-application create/update/void aliases translate ERP fields into the native PostgreSQL command projection, enter the submitted approval state, merge command rows into source-shaped readback, and preserve explicit no-cash/accounting/tax markers; imported rows remain read-only. |
| `cb_htfkplan`, `cb_htfk_apply`, `vcb_loan_simple`, `vcb_expense`, `sale_revenue` | Cashflow forecast, liquidity evidence, and explanation candidate | `finance/treasury` + `cmd/postgres_company_service/ai_explain_commands.mbt` + `scripts/company_postgres_ai_explain_smoke.sh` | Source-compatible `/api/company/cashflow/forecast`, `/forecast-v3`, `/forecast/detail`, `/inflow`, `/net`, and `/gap-alert` reads plus Rabbita `/cashflow` aggregate source planned, pending, approved, expense, loan, and revenue evidence. Signed `/ai-explain` (and `/source` alias) now returns a deterministic native summary over supplied series/gap evidence with `provider_execution=false`, `persisted=false`, and `authorizing=false`. Missing sales/milestone/CBS rows remain explicit empty-source state; cash release, bank settlement, accounting, tax, real provider execution, and finance-owner acceptance remain separate. See `docs/ERP_CASHFLOW_RUNTIME_AUDIT.md`. |
...
| `cb_cost`, `cb_subject_dict`, CBS versions | Dynamic/project cost | `finance/cost` + `finance/cbs` + `migration/erp` + `scripts/company_postgres_service.py` | `B = D + E + F + G` deterministic calculation and target/commitment/actual/progress forecast implemented; the reviewed fixture maps all 7 non-empty `cb_cost` rows to explicit active/frozen CBS subjects with durable projections, and an opt-in budget plan now records subject-scoped reservation/consumption evidence without posting. Source dynamic-cost create/update/void aliases now use a command projection and merge source-shaped rows while imported cost rows remain read-only; milestone state/trigger aliases are connected separately, while remarks writes, broader CBS enforcement, accounting, cash, tax, and period close remain separate. |
| `cb_plan_version`, `cb_r_master`, `cb_subject_dict`, `cb_contract`, `cb_expense_split`, `vcb_expense`, `cb_change_apply`, `sale_revenue` | ERP cost-dashboard v3 hierarchy/read boundary | `scripts/company_postgres_service.py` + `frontend/main` | Source-compatible `/api/company/investment/projects/:id/profit-actual-v2` preserves R/l2/l3 CBS rows and B/D/E/F/G/H cost calculations with explicit empty-source coverage; Rabbita `/cost-dashboard-v3` renders source rows when present and keeps the designer dashboard only for transport fallback. Budget reservations, accounting, cash, tax, and mutation authority remain separate. |
| `cb_r_master`, `cb_subject_dict`, `cb_plan_version`, `cb_contract`, `cb_change_apply`, `wf_approval_rule` | CBS master data, budget classification, R0, and approval evidence | `finance/cbs` + `scripts/company_postgres_service.py` | Source-compatible read-only `/api/company/cbs/*` family and Rabbita `/cbs/*` preserve dictionary/version/F-balance/R0/approval/change concepts with explicit coverage and `authorizing=false`. Current export has two unclassified contracts but no CBS dictionary/version/rule rows; mutations and accounting/budget ownership remain gated. |
| `fund_plan`, `fund_dispatch` | Fund planning, liquidity gap, and inter-project dispatch | `finance/treasury` + `scripts/company_postgres_service.py` | Source-compatible read-only `/api/company/fund/{plans,gap-analysis,dispatches}` reads and Rabbita `/fund/plan` preserve planned/actual net and dispatch evidence with explicit empty-source coverage. Cash release, dispatch approval, accounting, tax, and ownership remain gated. |
| `sys_warning`, warning rules, scans, tickets | Quality warning, exception observation, and follow-up | `governance/quality` + `scripts/company_postgres_service.py` | Source-compatible observed `/api/company/warning/*` reads evaluate twelve ERP rule definitions over imported envelopes, preserve one W005 source observation in the current export, and mark results `persisted=false`/`authorizing=false`. Signed warning-to-ticket now persists a command-owned `warning_ticket` candidate and `/tickets/mine` merges imported/local rows with assignee filtering; status, reassignment, and extension transitions append replayable local revisions, while notification delivery, provider execution, and production ownership remain gated. |
| ERP warning-rule governance routes | Built-in/custom warning configuration, custom-rule and scan previews, and ticket candidates | `governance/quality` + `cmd/postgres_company_service/warning.mbt` + `cmd/postgres_company_service/warning_ticket_commands.mbt` + `scripts/company_postgres_warning_smoke.sh` | Signed super-user `PATCH /api/company/warning/rules/:code`, `POST/DELETE /custom-rules[/:code]`, dry-run `POST /custom-rules/preview` and `/scan`, signed `POST /:guid/to-ticket`, and signed `PATCH /tickets/:id/{status,reassign,extend}` (plus `/source` aliases) persist idempotent redacted rule/ticket projections and audit receipts or return deterministic source coverage. Custom SQL is validated and hashed but never executed; both previews never persist findings, and ticket commands never send notifications/webhooks or invoke an external ticket provider. Completing a local ticket also writes a warning-state resolution overlay. Production scans, external delivery, and warning-owner acceptance remain gates. |
...
| `vcb_expense`, `cb_expense_detail`, `cb_expense_split` | ERP expense list/detail read boundary | `scripts/company_postgres_service.py` + `frontend/main` | Source-compatible `/api/company/budget/expenses` and `/api/company/budget/expenses/:guid` reads preserve imported master/detail/allocation envelopes, source coverage, and explicit empty-source state; Rabbita `/expenses` and `/expenses/:guid` remain read-only for imported rows and keep designer fixtures only for transport fallback. |
...
| ERP delivery/progress runtime routes | Project progress, output value, and task-report runtime | `operations/delivery` + `operations/project` + `cmd/postgres_company_service` + `scripts/company_postgres_delivery_smoke.sh` + `frontend/main` | Native progress/deliverable state machines and draft/recognition cohorts are tested. The compiled MoonBit PostgreSQL service and Rabbita `/project/progress` and `/project-plan` expose source-preserving task/report reads plus evidence-gated progress/output/task-report commands with idempotent replay; command-owned progress also has an idempotent tombstone/delete boundary. Imported rows remain read-only; the available export has no `proj_progress`/`proj_output` rows, and browser production-identity acceptance plus owner sign-off remain pending. Keep acceptance, output confirmation, cost, recognition, cash, tax, and close separate. See `docs/ERP_DELIVERY_RUNTIME_AUDIT.md`. |
...
| ERP workflow-definition runtime routes | Process definitions, ordered steps, and assignee configuration | `operations/workflow` + `scripts/company_postgres_service.py` + `frontend/main` | `/api/company/workflow/process-defs` and preview preserve two source definitions, twelve steps, and six assignees; Rabbita `/tasks` exposes them as non-authorizing read evidence. The source has zero workflow instances/actions, so task, approval, delegation, and synchronization behavior remain gated. See `docs/ERP_WORKFLOW_RUNTIME_AUDIT.md`. |
| ERP project master/plan runtime routes | Project master, lifecycle, task, report, key-node summary, delay-impact evidence, and AI plan suggestion candidate | `operations/project` + `operations/delivery` + `cmd/postgres_company_service` + `cmd/postgres_company_service/plan_ai_commands.mbt` + `scripts/company_postgres_project_plan_smoke.sh` + `frontend/main` | `/api/company/projects` and detail preserve two source projects, fourteen lifecycle instances, nine tasks, and one task report; source-compatible lifecycle/plan reads preserve seven ordered stages, seven tasks, one report, five key nodes, and downstream delay impact. Local project-plan task create/update/delete and evidence-gated report commands persist separate PostgreSQL projections with source/command provenance; imported task state is not mutated. Signed `/api/company/plan/ai-suggest-plan` now returns seven deterministic milestone nodes and end dates without provider execution or persistence. Rabbita `/projects` and `/projects/:guid` show the live rows while retaining the designer fallback. Provider execution, browser identity, and operations-owner acceptance remain separate gates. See `docs/ERP_PROJECT_RUNTIME_AUDIT.md`. |
| ERP MDM organization/project runtime routes | Business-unit hierarchy, legal-name evidence, and project lifecycle commands | `operations/mdm` + `scripts/company_postgres_service.py` | `/api/company/business-units/tree` preserves one group, two companies, four departments, and seven source rows with parent/child structure. `POST /api/company/source/mdm/projects` plus `PUT`/`DELETE .../:guid` now persist command-owned source-shaped project projections with seven pending lifecycle stages, imported-project protection, idempotent replay, audit receipts, and no task/workflow/budget/accounting/cash/tax effects. Organization ownership and browser/production identity acceptance remain separate. See `docs/ERP_MDM_RUNTIME_AUDIT.md`. |
| ERP budget dictionary runtime routes | Cost-subject and proceedings dictionaries | `operations/budget` + `scripts/company_postgres_service.py` | `/api/company/budget/dict/cost-subjects` and `/api/company/budget/proceedings` preserve five enabled cost-subject options and three enabled proceedings for source-compatible expense-form reads. Expense and dictionary mutations remain separate. See `docs/ERP_BUDGET_RUNTIME_AUDIT.md`. |
| ERP investment runtime routes | Feasibility versions, plan indices, profit summary, dimensions, Excel upload gate, index/plan-line, subject-mapping, and plan-line-edit dry-run candidates, plus explanation candidate | `investment/portfolio` + `cmd/postgres_company_service/investment_commands.mbt` + `cmd/postgres_company_service/investment_excel.mbt` + `cmd/postgres_company_service/ai_explain_commands.mbt` | Source-compatible investment reads preserve one current version, 26 indices in five dimensions, source profit values, and dimension metadata. Signed local version/index lifecycle commands now persist idempotent candidate projections and audit evidence while protecting imported rows. The signed workbook upload route now returns an explicit multipart/binary-parser gate; signed `/excel-imports/:id/index-upsert` and `/plan-lines/import`, `PUT /projects/:id/subject-mappings`, and `PUT /plan-lines/:id` (and `/source` aliases) return source-shaped dry-run plans; non-dry-run index, plan-line, mapping, or edit writes are explicit investment-owner gates with no persistence. Signed `/projects/:id/ai-explain` (and `/source` alias) deterministically summarizes the current PostgreSQL version/profit summary with no provider, persistence, or financial effect. Excel parser, valuation, cash, accounting, tax, provider execution, browser identity, and investment-owner acceptance remain separately authorized. See `docs/ERP_INVESTMENT_RUNTIME_AUDIT.md`. |
| ERP admin governance runtime routes | Dictionary groups/options, `sys_param`, audit log/actions, and health coverage | `foundation/access` + `intelligence/notification` + `cmd/postgres_company_service/admin_commands.mbt` + `scripts/company_postgres_admin_sys_param_smoke.sh` | Source-compatible admin reads preserve one dictionary group, five options, two audit rows, action counts, 29 health-table rows, and an explicitly non-authorizing empty BPM pool. Dictionary POST/PATCH and generic `sys_param` POST now use authenticated, idempotent command-owned overlays that preserve imported rows, store only redacted value digests, and write projection/receipt/audit evidence; provider execution, browser action wiring, super-user scope, audit writes/retention, role changes, and source completeness remain separate. See `docs/ERP_ADMIN_RUNTIME_AUDIT.md`. |
| ERP profile/preference runtime routes | Imported identity/profile and self-scoped user preferences | `foundation/organization` + `foundation/access` + `scripts/company_postgres_service.py` + `scripts/company_postgres_dev_gateway.py` | `/api/company/auth/me`, `/my-initiated`, `/prefs`, and RBAC observations preserve imported identity and explicit source coverage. Signed-user `PUT`/`DELETE /api/company/source/auth/prefs/:key` now persist idempotent local `user_preference` revisions with audit receipts and source-shaped readback; imported `sys_user_pref` rows stay read-only, and no authority, provider, accounting, cash, or tax effect is enabled. Production issuer/session durability, browser acceptance, role administration, and security-owner approval remain gates. See `docs/ERP_PROFILE_RUNTIME_AUDIT.md`. |
| ERP RBAC user-management runtime routes | User create/edit/enable/reset candidates with credential-safe audit | `foundation/access` + `cmd/postgres_company_service/rbac_commands.mbt` + `scripts/company_postgres_rbac_smoke.sh` | Signed super-user `POST/PUT /api/company/rbac/users`, `/toggle`, and `/reset-password` now persist redacted `rbac_user` candidates with idempotent command receipts and audit events. Imported identity rows remain read-only; password material is stored only as a SHA-256 digest, `authorizing=false`, and production identity/token binding, effective authorization, and browser owner acceptance remain gates. |
| ERP employee-loan runtime routes | Employee advances, loan balances, and offset evidence | `finance/employee_finance` + `cmd/postgres_company_service` + `scripts/company_postgres_loan_smoke.sh` + `frontend/main` | Compiled MoonBit `/api/company/loans` and detail reads preserve the exported `vcb_loan_simple`/`cb_loan_offset` rows and feed Rabbita `/loans`; imported balances and offsets remain read-only. Local create, submit, bounded offset, draft update, and draft/rejected void now persist authority-bearing, idempotent, audited `employee_advance` command projections. Workflow synchronization remains gated until `wf_process_instance` source rows and workflow ownership are supplied; Python remains frozen comparison evidence only. See `docs/ERP_LOAN_RUNTIME_AUDIT.md`. |
| ERP attachment runtime routes | Attachment metadata, AI extraction status, upload/delete gates, evidence linkage, and OCR re-extraction candidate | `foundation/evidence` + `cmd/postgres_company_service/attachment_commands.mbt` + `scripts/company_postgres_attachment_smoke.sh` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `frontend/main` | `/api/company/attachments/{list,all,stats}` preserve imported metadata and source coverage; the current export has no `attachment` rows, so Rabbita `/attachments` renders an explicit empty source state. Signed `POST /upload`, `DELETE /:guid`, and `POST /re-extract/:guid` (and `/source` aliases) return explicit non-authorizing object-store/OCR candidates without multipart reads, binary storage/deletion, OCR calls, or metadata persistence. Binary ownership/download, provider-backed OCR, retention, and authorization remain separate gates. See `docs/ERP_ATTACHMENT_RUNTIME_AUDIT.md`. |
| ERP marketing runtime routes | Campaigns, placements, channels, and materials | `operations/marketing` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `frontend/main` | `/api/company/marketing/{campaigns,placements,channels,materials}` preserve source metadata and coverage; the current export has no marketing rows, so Rabbita `/marketing` renders an explicit empty source state while the reviewed synthetic cohort remains separate. Marketing commands, spend/CBS consumption, accounting, attribution, and owner acceptance remain gates. See `docs/ERP_MARKETING_RUNTIME_AUDIT.md`. |
| ERP notification runtime routes | Company-owned messages, subscriptions, redacted notification config, email metadata, digest evidence, and provider discovery | `intelligence/notification` + `cmd/postgres_company_service/notification_commands.mbt` + `cmd/postgres_company_service/notification_dispatch_commands.mbt` + `cmd/postgres_company_service/notification_email_commands.mbt` + `cmd/postgres_company_service/ai_provider_commands.mbt` + `scripts/company_postgres_notification_smoke.sh` + `scripts/company_postgres_ai_provider_smoke.sh` + `frontend/main` | `/api/company/notify/{messages,messages/unread-count,subscriptions,config,email-outbox,digest/preview,digest/log,llm-providers}` preserve user-scoped message/unread reads, subscription rows, redacted config status, email metadata, digest evidence, and provider discovery with explicit source coverage. Signed-user source subscription create/update/delete aliases and message read/read-all aliases now persist idempotent local projections with source-shaped readback, replay/audit evidence, tombstones, and imported-message read overlays; signed super-user `POST /digest/dispatch` candidates persist bounded warning/recipient receipts, `POST /config/test-webhook` candidates persist effective URL/kind state with `wouldSend`/skip reasons, email test/redelivery candidates verify recipient/outbox evidence with `wouldQueue`/`wouldRedeliver`, and `/llm-test` records redacted provider-test candidates; all remain dry-run/no delivery and no provider call. Imported `sys_warning_subscription`, `sys_message`, and `sys_email_outbox` rows remain read-only and delivery/provider/accounting/cash/tax effects remain false. The current export has no notification rows, so Rabbita `/inbox` and `/notify-config` render empty source states after successful reads. Configuration writes, delivery, provider execution, consent, and owner acceptance remain gates. See `docs/ERP_NOTIFICATION_RUNTIME_AUDIT.md`. |
| ERP OCR/error-log runtime routes | OCR provider status, scene/configuration metadata, and bounded service-error evidence | `intelligence/ai` + `foundation/evidence` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `frontend/main` | `/api/company/admin/ocr/status` and `/api/company/admin/error-log` preserve provider definitions and redacted key status without provider execution, plus bounded error metadata with IP/stack redaction. The current export has no `sys_param` or `sys_error_log` rows, so Rabbita `/ocr-config` and `/error-log` render explicit definition/empty-source states. OCR calls, config writes, retention, production identity, and super-user ownership remain gates. See `docs/ERP_ADMIN_RUNTIME_AUDIT.md`. |
| ERP AI provider test runtime routes | OCR/LLM provider connectivity test candidates | `cmd/postgres_company_service/ai_provider_commands.mbt` + `scripts/company_postgres_ai_provider_smoke.sh` + `frontend/main` | Signed super-user `POST /api/company/admin/ocr/test`, `/api/company/admin/llm/test`, and notification `/llm-test` candidates preserve provider/model/key/endpoint configuration evidence with digests and idempotent audit receipts; responses are `dryRun=true`, `tested=false`, and `providerExecution=false`, with no credential value or external call. Production provider execution, browser acceptance, and security-owner approval remain gates. |
| ERP AI analytics runtime routes | Confirmed AI drafts, query activity, corrections, workflow auto-skips, and AI-origin badge evidence | `cmd/postgres_company_service/observations.mbt` + `cmd/postgres_company_service/main.mbt` + `scripts/company_postgres_ai_stats_smoke.sh` + `frontend/main` | `/api/company/ai-stats/{overview,activity,badge}` and `POST /badge/batch` (plus `/source`) preserve source analytics and optional user labels with explicit coverage, `authorizing=false`, `persisted=false`, and `provider_execution=false`. Batch lookup selects confirmed imported drafts by bounded business GUIDs and returns the source map shape without AI execution. The current export has no AI analytics rows, so Rabbita `/ai-stats` renders empty-source state after successful reads. LLM/OCR execution, draft promotion, workflow authority, prompt retention, and owner acceptance remain gates. See `docs/ERP_AI_RUNTIME_AUDIT.md`. |
| ERP AI Hub runtime routes | AI workbench usage, draft history, query history, correction logs, correction-rate observation, explanation, and command-gate candidates | `intelligence/ai` + `cmd/postgres_company_service/ai_hub_commands.mbt` + `cmd/postgres_company_service/ai_hub_command_commands.mbt` + `scripts/company_postgres_ai_hub_smoke.sh` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `frontend/main` | Source-compatible `/api/company/ai-hub/{usage-stats,drafts,drafts/:draftId,query-log,corrections,correction-stats}` preserve the source response shapes with explicit coverage and redacted draft/query payloads. Signed `POST /api/company/ai-hub/explain` (and `/source`) returns a deterministic table summary with `provider_execution=false`, `persisted=false`, and `authorizing=false`; the nine mutation routes now return a signed command-gate candidate with no request-body consumption, provider/query execution, draft persistence, workflow authority, or financial effect. The current export has no AI Hub draft/query/correction/session rows, so Rabbita `/ai-hub` loads successful empty-source metrics/history while retaining the designer workbench. Provider-backed explanation, prompt retention, durable command mutation, and draft promotion remain gated. See `docs/ERP_AI_RUNTIME_AUDIT.md`. |
| ERP webhook configuration, test candidate, and overdue-scan runtime routes | Three-platform Feishu/DingTalk/WeCom webhook configuration metadata, candidate update/test/scan, and dry-run overdue preview | `cmd/postgres_company_service/webhook_commands.mbt` + `cmd/postgres_company_service/webhook_delivery_commands.mbt` + `cmd/postgres_company_service/webhook_scan_commands.mbt` + `scripts/company_postgres_webhook_smoke.sh` + `frontend/main` | `/api/company/webhook/config` and `/source` preserve the source `sys_param` shape with redacted URL/secret status and explicit coverage; signed super-user `PUT /config/:platform` candidates preserve enabled/optional URL/secret/`__keep__` semantics with idempotent redacted projections and audit evidence, but do not bind credentials or deliver. Signed `POST /test/:platform` candidates validate effective platform state and persist redacted dry-run receipts with `wouldSend`/skip reasons while provider execution remains false. Signed `POST /scan-overdue` candidates reuse the imported ticket window, persist bounded scan receipts, and return `dryRun=true`, `sent=false`, and `ticketMutation=false`; `/scan-overdue/preview` remains the read-only counterpart. The current export has no `sys_param` or warning-ticket rows, so Rabbita `/webhook-config` renders empty-source state after a successful read. Managed credential storage, actual provider delivery, production overdue mutation, production identity, and owner acceptance remain gates. See `docs/ERP_WEBHOOK_RUNTIME_AUDIT.md`. |
| ERP report-builder runtime routes | Report table/column/operator whitelist and saved report templates | `finance/reporting` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `frontend/main` | `/api/company/reports/templates/meta` preserves ten source table definitions and eight operators; `/api/company/reports/templates` preserves source template shape and explicit `sys_report_template` coverage. The current export has no saved templates, so Rabbita `/report-builder` renders empty-source template state while retaining the designer builder. Template execution, writes, deletion, exports, production identity, and report-owner acceptance remain gates. See `docs/ERP_REPORT_BUILDER_RUNTIME_AUDIT.md`. |
