...
| ERP employee-loan runtime routes | Employee advances, loan balances, and offset evidence | `finance/employee_finance` + `scripts/company_postgres_service.py` + `frontend/main` | `/api/company/loans` and detail reads preserve the exported `vcb_loan_simple`/`cb_loan_offset` rows and feed Rabbita `/loans`; imported balances and offsets remain read-only. Create, submit, offset, workflow-sync, update, and void commands still require a separate authority/idempotency/audit slice. See `docs/ERP_LOAN_RUNTIME_AUDIT.md`. |
