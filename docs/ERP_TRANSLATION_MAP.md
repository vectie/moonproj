...
| ERP notification runtime routes | Company-owned messages, subscriptions, redacted notification config, email metadata, digest evidence, and provider discovery | `intelligence/notification` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `frontend/main` | `/api/company/notify/{messages,messages/unread-count,subscriptions,config,email-outbox,digest/preview,digest/log,llm-providers}` preserve user-scoped message/unread reads, subscription rows, redacted config status, email metadata, digest evidence, and provider discovery with explicit source coverage. The current export has no notification rows, so Rabbita `/inbox` and `/notify-config` render empty source states after successful reads. Acknowledgement, subscription/configuration writes, delivery, provider execution, consent, and owner acceptance remain gates. See `docs/ERP_NOTIFICATION_RUNTIME_AUDIT.md`. |
