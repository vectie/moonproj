...
| `tender_plan`, `tender_award` | Tender planning, bidding, and award | `operations/procurement` + `scripts/company_postgres_service.py` | Planning → publishing → bidding → award → completion flow with qualified-supplier and duplicate-bid guards; the reviewed procurement cohort persists bid/award evidence and a separate performed commitment without settlement. The Rabbita `/tender` route now reads PostgreSQL tender projections and drives local planning/publish/open-bidding/cancel commands; source-compatible create and split aliases translate ERP field names into local command projections and merge them back with provenance. Award remains gated on supplier qualification and matching bid evidence, while source arbitrary-state overwrite and imported deletion remain policy gates. |
