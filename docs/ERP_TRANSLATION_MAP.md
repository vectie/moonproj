...
| ERP investment runtime routes | Feasibility versions, plan indices, profit summary, dimensions, Excel index/plan-line, subject-mapping, and plan-line-edit dry-run candidates, plus explanation candidate | `investment/portfolio` + `cmd/postgres_company_service/investment_commands.mbt` + `cmd/postgres_company_service/investment_excel.mbt` + `cmd/postgres_company_service/ai_explain_commands.mbt` | Source-compatible investment reads preserve one current version, 26 indices in five dimensions, source profit values, and dimension metadata. Signed local version/index lifecycle commands now persist idempotent candidate projections and audit evidence while protecting imported rows. Signed `/excel-imports/:id/index-upsert` and `/plan-lines/import`, `PUT /projects/:id/subject-mappings`, and `PUT /plan-lines/:id` (and `/source` aliases) now return source-shaped dry-run plans; non-dry-run index, plan-line, mapping, or edit writes are explicit investment-owner gates with no persistence. Signed `/projects/:id/ai-explain` (and `/source` alias) deterministically summarizes the current PostgreSQL version/profit summary with no provider, persistence, or financial effect. Excel upload/import, valuation, cash, accounting, tax, provider execution, browser identity, and investment-owner acceptance remain separately authorized. See `docs/ERP_INVESTMENT_RUNTIME_AUDIT.md`. |
