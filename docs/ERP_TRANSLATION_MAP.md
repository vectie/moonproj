...
| ERP admin governance runtime routes | Dictionary groups/options, audit log/actions, and health coverage reads | `foundation/access` + `intelligence/notification` + `scripts/company_postgres_service.py` | Source-compatible admin reads preserve one dictionary group, five options, two audit rows, action counts, 29 health-table rows, and an explicitly non-authorizing empty BPM pool. Super-user scope, dictionary/audit writes, role changes, retention, and source completeness remain separate. See `docs/ERP_ADMIN_RUNTIME_AUDIT.md`. |
