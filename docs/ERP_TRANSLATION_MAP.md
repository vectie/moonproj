...
| ERP employee-loan runtime routes | Employee advances, loan balances, and offset evidence | `finance/employee_finance` + `scripts/company_postgres_service.py` + `frontend/main` | `/api/company/loans` and detail reads preserve the exported `vcb_loan_simple`/`cb_loan_offset` rows and feed Rabbita `/loans`; imported balances and offsets remain read-only. Local create, submit, bounded offset, draft update, and draft/rejected void now persist authority-bearing, idempotent, audited `employee_advance` command projections. Workflow synchronization remains gated until `wf_process_instance` source rows and workflow ownership are supplied. See `docs/ERP_LOAN_RUNTIME_AUDIT.md`. |
