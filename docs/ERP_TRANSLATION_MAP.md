...
| ERP project master/plan runtime routes | Project master, lifecycle, task, report, and key-node summary evidence | `operations/project` + `operations/delivery` + `scripts/company_postgres_service.py` + `frontend/main` | `/api/company/projects` and detail preserve two source projects, fourteen lifecycle instances, nine tasks, and one task report; source-compatible `/projects/:id/tasks`, `/tasks/:id`, and `/projects/:id/plan-summary` reads preserve seven tasks, one report, and five key nodes. Rabbita `/projects` and `/projects/:guid` show the live rows while retaining the designer fallback. Project and plan mutations remain separate gates. See `docs/ERP_PROJECT_RUNTIME_AUDIT.md`. |
