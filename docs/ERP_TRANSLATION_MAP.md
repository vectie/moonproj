...
| ERP notification runtime routes | Company-owned messages, subscriptions, redacted notification config, email metadata, digest evidence, and provider discovery | `intelligence/notification` + `scripts/company_postgres_service.py` + `scripts/company_postgres_read_model_server.py` + `scripts/company_postgres_dev_gateway.py` + `frontend/main` | `/api/company/notify/{messages,messages/unread-count,subscriptions,config,email-outbox,digest/preview,digest/log,llm-providers}` preserve user-scoped message/unread reads, subscription rows, redacted config status, email metadata, digest evidence, and provider discovery with explicit source coverage. Signed-user source subscription create/update/delete aliases now persist idempotent local `notification_subscription` projections with source-shaped readback, replay/audit evidence, and tombstones; imported `sys_warning_subscription` rows remain read-only and delivery/provider/accounting/cash/tax effects remain false. The current export has no notification rows, so Rabbita `/inbox` and `/notify-config` render empty source states after successful reads. Acknowledgement, configuration writes, delivery, provider execution, consent, and owner acceptance remain gates. See `docs/ERP_NOTIFICATION_RUNTIME_AUDIT.md`. |
