...
| `cb_contract` | Commercial commitment | `operations/commitment` + `migration/erp` + `scripts/company_postgres_service.py` | Commitment state machine implemented; promotion requires explicit principal, project-scope, counterparty, and amount-bounded authority mappings. Source contract update/void aliases now reuse a command-owned PostgreSQL contract projection, preserve source-shaped fields and tombstone readback, and keep imported contracts read-only; source contract creation, CBS/budget validation, and accounting/cash/tax effects remain separate gates. A separate reviewed accounting-link plan can persist source-to-journal identity without posting cash. |
